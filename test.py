...
from pathlib import Path  # ← 跨平台路径解析
...
def infer_CFM(args):
    ...
    for (rgb, pc, depth), gt, label, rgb_path in tqdm(test_loader,
        ...
        with torch.no_grad():
            ...
            # 将特征视作 [B, D, N]，而上面训练里用的是 [B, N, D]，对齐下面计算
            # 此处你的 feature_extractor 输出与训练一致（前面训练中直接用 metric 维度是 -1）
            # 这里延续训练时做法：沿通道 dim=1 归一化后求 L2 差
            ...
            # 将向量恢复为 2D 图（假设 N 是 224*224/patchsize^2 的网格；你的训练里没有重排到 224，
            # 但原绘图逻辑用 224×224，这里保持一致：直接 reshape 到 224x224）
            B, N = det_vec.shape
            det_map = det_vec.reshape(224, 224)  # 单张图情形（B=1）

            # 同时保留原 cos_2d/3d 和乘积，便于对比/可视化
            ...
            # ---------------- 可视化（保持原 3x3 格式；右上角仍显示原 cos_comb 作为对比） ----------------
            if args.produce_qualitatives:
                rgb_path_str = str(rgb_path[0])
                p = Path(rgb_path_str)
                image_name_str = p.name
                defect_class_str = p.parent.parent.name  # ★ 修正：缺陷类型在 test/<defect>/rgb/xxx.png
    ...
    # Calculate AD&S metrics（与原一致：对 det_map 的预测评估）
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_folder',
                        default='F:/代写论文/SCI闭源4代码/改进代码/checkpoints_CFM_mvtec-20251017T151242Z-1-001/checkpoints_CFM_mvtec',
                        type=str,
